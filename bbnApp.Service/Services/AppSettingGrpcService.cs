...
namespace bbnApp.Service.Services
{
    public class AppSettingGrpcService: AppSettingGrpc.AppSettingGrpcBase
    {
        ...
        /// <returns></returns>
        [Authorize]
        public override async Task<AppSettingDownloadResponse> AppSettingDownload(AppSettingDownloadRequest request, ServerCallContext context)
        {
            AppSettingDownloadResponse? response = null;
            try
            {
                if (context.UserState.TryGetValue("User", out var userObj) && userObj is UserModel user)
                {
                    var (code, msg, result) = await _appSettingService.AppSettingDownLoad();
                    response = new AppSettingDownloadResponse
                    {
                        Code = code,
                        Message = msg
                    };
                    response.Items.AddRange(_mapper.Map<List<AppSetting>>(result));
                }
                else
                {
                    response = new AppSettingDownloadResponse
