...
namespace bbnApp.deskTop.ViewModels
{
    public partial class MainWindowViewModel : ViewModelBase
    {
        ...
        private void DicInit()
        {
            try
            {
                dialog.ShowLoading("字典初始化中", async (e) =>
                {
                    var header = CommAction.GetHeader();
                    await AreaCodeInit(header);
                    dialog.LoadingClose(e);
                });
        ...
        {
            AreaTreeNodeRequest request = new AreaTreeNodeRequest { AreaCode = string.Empty, AreaLeve = 0 };
            AreaTreeNodeResponse response =await _codeClient.AreaTreeLoadAsync(request,header);
            if(response.Code)
            {
                List<AreaTreeNodeDto> list = _mapper.Map<List<AreaTreeNodeDto>>(response.Items);
                AreaTreeContext.AreaTree = list;
            }
            else
            {
