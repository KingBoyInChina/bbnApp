...
namespace bbnApp.Application.Services.CODE
{
    public class DataDictionaryService:IDataDictionaryService
    {
        ...
        public async Task DicInit()
        {
            try
            {
                var list = await dapperRepository.QueryAsync<TreeItem>($"select DicCode as Id,DicPCode as PId,DicName as Name,Remarks as Tag,True as IsLeaf from {StaticModel.DbName.bbn_code}.datadictionar where Isdelete=0 and IsLeaf=1 and IsLock=0 order by DicIndex asc");
                foreach(var item in list)
                {
                    var children = await dapperRepository.QueryAsync<TreeItem>($"select ItemId as Id,DicCode as PId,ItemName as Name,ReMarks as Tag,true as IsLeaf from {StaticModel.DbName.bbn_code}.datadictionarylist where Isdelete=0 and IsLock=0 order by ItemIndex asc;");
